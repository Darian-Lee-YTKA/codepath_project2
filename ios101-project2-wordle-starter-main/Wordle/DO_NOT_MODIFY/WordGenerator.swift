...
enum WordTheme: String {
  case normal, animals, countries, turkish
  ...
  func possibleWords(numLetters: Int) -> [String] {
    ...
    case .turkish:
        return turkishWords(with: numLetters)
    }
  }
  ...
  func countryWords(with numLetters: Int) -> [String] {
    ...
    func turkishWords(with numLetters: Int) -> [String] {
      if numLetters == kMinLetters {
        return ["KEDI", "ELMA", "YARA"]
      } else if numLetters == kMinLetters + 1 {
        return ["KALEM", "ORMAN", "TAVUK"]
      } else if numLetters == kMinLetters + 2 {
        return ["ELBISE", "GOZLUK", "YAPRAK"]
      } else if numLetters == kMaxLetters {
        return ["EVLILIK", "MERHABA", "RESIMLER"]
      }
      return []
    }
}

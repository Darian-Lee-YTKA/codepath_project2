...
class BoardController: NSObject,
  ...
  func resetBoardWithCurrentSettings() {
      ...
      applyNumLettersSettings(with: currentSettings)
      applyNumGuessesSettings(with: currentSettings)
      applyIsAlienWordleSettings(with: currentSettings)
      numTimesGuessed = 0
      collectionView.reloadData()
  ...
  private func applyThemeSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
      print(goalWord)
      if let themeAsString = settings[kWordThemeKey] as? String {
          print(themeAsString)
          if let themeAsTheme = WordTheme(rawValue: themeAsString){
              print(themeAsTheme)
              goalWord = WordGenerator.generateGoalWord(with: themeAsTheme)}
  ...
  private func applyIsAlienWordleSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
      //print(settings)
      if let AlienWordleVal = settings[kIsAlienWordleKey] as? Bool {
          isAlienWordle = AlienWordleVal
      }
  }
}
